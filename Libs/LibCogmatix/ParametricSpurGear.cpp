...
{
	ParametricSpurGear::ParametricSpurGear(short numberOfTeeth, double depth, double axisDiameter, double module, double helix)
		: _numberOfTeeth(numberOfTeeth), _depth(depth), _axisDiameter(axisDiameter), _module(module), _helix(helix)
	{
...
EXCEPT_IF (module < fabs(epsilon), CogException::BadParameter, "Bad module");
		...
		for (short i=0; i < numberOfEdges; ++i)
		{
			...
			double xp1 = _rootDiameter * cos (2.*pi * double (i+1)/numberOfEdges);
			double yp1 = _rootDiameter * sin (2.*pi * double (i+1)/numberOfEdges);

			double x0z = _rootDiameter * cos (_helix + 2.*pi * double (i)/numberOfEdges);
			double y0z = _rootDiameter * sin (_helix + 2.*pi * double (i)/numberOfEdges);
			double xp1z = _rootDiameter * cos (_helix + 2.*pi * double (i+1)/numberOfEdges);
			double yp1z = _rootDiameter * sin (_helix + 2.*pi * double (i+1)/numberOfEdges);
			...
			{ 
				double xm1 = _rootDiameter * cos (2.*pi * double (i-1)/numberOfEdges);
				double ym1 = _rootDiameter * sin (2.*pi * double (i-1)/numberOfEdges);
				double xp2 = _rootDiameter * cos (2.*pi * double (i+2)/numberOfEdges);
				double yp2 = _rootDiameter * sin (2.*pi * double (i+2)/numberOfEdges);
		...
		{
			double xm1 = _axisDiameter * cos (2.*pi * double (i-1)/numberOfEdges);
			double ym1 = _axisDiameter * sin (2.*pi * double (i-1)/numberOfEdges);
			double x0 = _axisDiameter * cos (2.*pi * double (i)/numberOfEdges);
			double y0 = _axisDiameter * sin (2.*pi * double (i)/numberOfEdges);
			double xp1 = _axisDiameter * cos (2.*pi * double (i+1)/numberOfEdges);
			double yp1 = _axisDiameter * sin (2.*pi * double (i+1)/numberOfEdges);
		...
		for (short j=0; j < 2; ++j)
		{
			...
			for (short i=0; i < numberOfEdges+1; ++i)
			{
				double x0 = _axisDiameter * cos (2.*pi * double (i)/numberOfEdges);
				double y0 = _axisDiameter * sin (2.*pi * double (i)/numberOfEdges);
				double x1 = _rootDiameter * cos (2.*pi * double (i+1)/numberOfEdges);
				double y1 = _rootDiameter * sin (2.*pi * double (i+1)/numberOfEdges);
				
				double x0z = _axisDiameter * cos (_helix + 2.*pi * double (i)/numberOfEdges);
				double y0z = _axisDiameter * sin (_helix + 2.*pi * double (i)/numberOfEdges);
				double x1z = _rootDiameter * cos (_helix + 2.*pi * double (i+1)/numberOfEdges);
				double y1z = _rootDiameter * sin (_helix + 2.*pi * double (i+1)/numberOfEdges);
